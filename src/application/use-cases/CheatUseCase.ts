...
export class CheatUseCase {
    ...
    async execute(mezonId: string, petId: string, exp: number, level: number): Promise<Pet> {
        Logger.info(`Thực hiện cheat cho thú cưng ${petId} của người dùng ${mezonId}`);
        ...
        if (!PetValidator.isPetReadyForAction(pet)) {
            Logger.warn(`Thú cưng ${petId} của người dùng ${mezonId} chưa sẵn sàng cho hành động`);
        ...
        Logger.info(`Đã thực hiện cheat thành công cho thú cưng ${petId} của người dùng ${mezonId}`);
        return updatedPet;
    }
}